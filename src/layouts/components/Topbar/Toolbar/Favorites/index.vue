  ...
  <div class="favorites-container w-80">
    <div class="flex items-center justify-between px-4 py-3">
      ...
      <template v-if="favoritesStore.canAdd(route.fullPath)">
        <SvgIcon v-if="!favoritesStore.isAdd(route.fullPath)" class="cursor-pointer text-xl" name="mdi:star-plus-outline" title="添加到收藏夹" @click="favoritesStore.add(route)" />
        <SvgIcon v-else class="cursor-pointer text-xl" name="mdi:star-remove" title="从收藏夹删除" @click="favoritesStore.remove(route.fullPath)" />
      </template>
    </div>
    <OverlayScrollbarsComponent :options="{ scrollbars: { autoHide: 'leave', autoHideDelay: 300 } }" defer class="max-h-220px">
      <div v-if="favoritesStore.list.length > 0" ref="favoritesContainerRef" class="flex flex-wrap items-center justify-between gap-2 px-4 pb-4">
        <div v-for="item in favoritesStore.list" :key="item.fullPath" :title="generateI18nTitle(item.i18n, item.title)" class="draggable-item relative w-[calc(50%-0.25rem)] flex cursor-pointer items-center gap-1 rounded px-2 py-2 ring-1 ring-stone-3 ring-inset hover:bg-stone-1 dark:ring-stone-7 dark:hover:bg-dark/50" @click="router.push(item.fullPath)">
          <SvgIcon v-if="item.icon" :name="item.icon" :size="18" />
          <div class="name flex-1 truncate pe-4">
            {{ generateI18nTitle(item.i18n, item.title) }}
          </div>
          <SvgIcon name="ep:delete" :size="14" class="right-1 h-5 w-6 rounded-full text-stone-3 !absolute dark:text-stone-7 hover:text-stone-7 dark:hover:text-stone-3" @click.stop="favoritesStore.remove(item.fullPath)" />
        </div>
      </div>
      <div v-else flex="center col" py-6 text-stone-5>
        ...
        <p v-show="favoritesStore.canAdd(route.fullPath)" m-0 flex-center text-sm op-75>
          点击右上角
          <SvgIcon name="mdi:star-plus-outline" :size="20" mx-1 text-stone-6 dark:text-stone-4 />
          将当前页面添加到收藏夹
        </p>
      </div>
