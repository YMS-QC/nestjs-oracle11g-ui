...
const themeList = computed(() => {
  return Object.keys(themes).map((key) => {
    ...
    return item.value['color-scheme'] === settingsStore.settings.app.colorScheme
  })
})
...
<template>
  <HSlideover v-model="isShow" title="应用配置">
    ...
    <div class="themes">
      <div v-for="item in themeList" :key="item.label" class="theme" :class="{ active: isDark ? settingsStore.settings.app.darkTheme === item.label : settingsStore.settings.app.lightTheme === item.label }" @click="isDark ? settingsStore.settings.app.darkTheme = item.label : settingsStore.settings.app.lightTheme = item.label">
    ...
    <div class="setting-item">
      ...
      <HToggle v-model="settingsStore.settings.topbar.switchTabbarAndToolbar" :disabled="!settingsStore.settings.tabbar.enable" />
    </div>
    <div class="divider">
      标签栏
    </div>
    ...
    <div v-if="settingsStore.mode === 'pc'" class="setting-item">
      ...
      <HToggle v-model="settingsStore.settings.breadcrumb.enableMainMenu" :disabled="!settingsStore.settings.breadcrumb.enable || ['single'].includes(settingsStore.settings.menu.menuMode)" />
    </div>
    <div class="divider">
      页面
    </div>
    ...
    <div class="setting-item">
      ...
      <HInput v-model="settingsStore.settings.home.title" :disabled="settingsStore.settings.toolbar.enableI18n" />
    </div>
    <div class="divider">
      其它
    </div>
