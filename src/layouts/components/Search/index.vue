...
const resultList = computed(() => {
  ...
  result = sourceList.value.filter((item) => {
    ...
    if (
      generateI18nTitle(item.i18n, item.title).toString().includes(searchInput.value)
      || match(generateI18nTitle(item.i18n, item.title).toString(), searchInput.value, { continuous: true })?.length > 0
    ) {
      flag = true
    }
    if (item.breadcrumb.some((b) => {
      return generateI18nTitle(b.i18n, b.title).toString().includes(searchInput.value)
        || match(generateI18nTitle(b.i18n, b.title).toString(), searchInput.value, { continuous: true })?.length > 0
    })) {
      flag = true
    }
    return flag
          ...
          <TransitionChild as="template" v-bind="transitionClass">
            <DialogPanel class="relative h-full max-h-4/5 w-full flex flex-col text-left sm:max-w-2xl">
              <HTabList
                ...
                ]" class="mb-4 flex!" @click.stop @change="switchType"
              />
              <div class="flex flex-col overflow-y-auto rounded-xl bg-white shadow-xl dark:bg-stone-8">
                <div class="flex items-center px-4 py-3" border-b="~ solid stone-2 dark:stone-7">
                  ...
                  <input ref="searchInputRef" v-model="searchInput" :placeholder="t('app.search.input')" class="w-full border-0 rounded-md bg-transparent px-3 text-base text-dark dark:text-white focus:outline-none placeholder-stone-4 dark:placeholder-stone-5" @keydown.esc="eventBus.emit('global-search-toggle')" @keydown.up.prevent="keyUp" @keydown.down.prevent="keyDown" @keydown.enter.prevent="keyEnter">
                </div>
